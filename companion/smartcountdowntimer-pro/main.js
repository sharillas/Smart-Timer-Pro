...
class SmartCountdownTimerProInstance extends InstanceBase {
	async init(config) {
		this.config = config;
	...
	initPresets() {
		...
		for (let i = 1; i <= 5; i++) {
			presets[`trigger_msg_${i}`] = {
				...
				style: {
					text: `[${i}]\\n$(smartcountdowntimer-pro:msg_${i})`,
					size: '14',
					color: combineRgb(255, 255, 255),
...
runEntrypoint(SmartCountdownTimerProInstance, []);
